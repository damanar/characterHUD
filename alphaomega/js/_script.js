...
function rollThis(num){
  ...
  if( typeof currentMenu.attr('quality') != "undefined" ){
    ...
    if( i != num-1 ){
      $('#current_roll').append(roll + "/" + dice[i] + " | ");
    }
    else {
      $('#current_roll').append(roll + "/" + dice[i] + "<br />");
    }
		result = result + roll;
	}

    $('#current_roll').append("result: " + result + " + modifier: " + mod + " = " + (result + mod));
...
function wieldpop() {
  if( $('#wielding').offset().left == -1000 ) {
    $('#wielding').animate({ left: 10 }, 1000, 'swing' ); 
  }
  else {
